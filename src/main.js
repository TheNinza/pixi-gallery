...
import backgroundFragmentShader from "./shader/background-shader.glsl?raw";
...
(function () {
  ...
  let lastImageClick;
  let imageClickStartTime;
  let imageClickEndTime;
  let imageClickStartPosition;
  let imageClickEndPosition;
  ...
  function initGrid() {
    ...
    gridColumns = gridColumnsCount * 20;
    gridRows = gridRowsCount * 20;
      ...
      if (rectIntersectsWithViewport(rect)) {
        ...
        if (!rect.discovered) {
          ...
          if (!image.interactive) {
            ...
            image.on("pointerdown", onImageDown);
            image.on("pointerup", onImageUp);
          }
        }
        ...
        // if (image.interactive) {
        //   image.off("pointerdown", onImageClick);
  ...
  function onImageDown(e) {
    lastImageClick = e.target?.texture?.baseTexture?.resource?.url;
    imageClickStartTime = Date.now();
    imageClickStartPosition = {
  ...
  function onImageUp(e) {
    imageClickEndTime = Date.now();
    imageClickEndPosition = {
      x: e.data.originalEvent.pageX,
      y: e.data.originalEvent.pageY,
    };

    const imageClickDuration = imageClickEndTime - imageClickStartTime;
    const imageClickDistance = distance(
      imageClickStartPosition,
      imageClickEndPosition
    );

    if (imageClickDuration < 500 && imageClickDistance < 10) {
      window.open(lastImageClick);
    }
  }
  ...
  function initEvents() {
    ...
    pointerStart.set(width / 2, height / 2);
    ...
    let update = () => {
      ...
      initX += 0.5 * xMultiplier;
      initY += 0.5 * yMultiplier;
      const { x, y } = {
        x: initX,
        y: initY,
      };
